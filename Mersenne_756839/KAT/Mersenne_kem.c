		   ...
		   const unsigned char *sk)
{
  ...
  unsigned char seed[32];
  unsigned char basic_sk[3*CRYPTO_BASIC_SECRETKEYBYTES];
  ...
  /* Re-generate expanded key from secret key in seed form */
  memset(seed,0,32);
  memcpy(seed,sk,SECURITYLVL/8);
  ...
  mersenne_kem_enc_seed(ct2, ss, basic_sk+CRYPTO_BASIC_SECRETKEYBYTES, kemseed);
  ...
  if (retcode) memset(ss,0,CRYPTO_BYTES);
